...
async def mark_session_token(token_hash: str, user_id: str, ts: float):
    ...
    await supabase_request("post", SUPABASE_SESSION_TABLE, payload=payload, prefer="resolution=merge-duplicates")
...
def render_page(
    ...
    favicon = (
        ...
        "%3Crect width='64' height='64' rx='12' fill='%2311151b'/%3E"
        "%3Cpath d='M20 50h24l4-24-10-8-4-6-4 6-10 8z' fill='%237c5cff'/%3E"
        "%3Cpath d='M26 46h12l3-18-9-6-6 5z' fill='%23e6e9ef'/%3E"
        "%3C/svg%3E"
    )
    return f"""
    <html>
...
async def callback(request: Request, code: str, state: str, lang: Optional[str] = None):
    ...
    try:
        if history:
            latest_status = history[0].get("status") or ""
            latest_ref = history[0].get("appeal_id") or ""
    except Exception:
        pass
    ...
    if already_appealed:
        status_line = "pending"
        if latest_status:
            status_line = latest_status
        blocked = f"""
          <div class="card status">
            <h2>Appeal already submitted</h2>
            <p class="muted">You can submit only one appeal for this ban. Current status: <strong>{html.escape(status_line)}</strong>{' (ref ' + html.escape(latest_ref) + ')' if latest_ref else ''}.</p>
        ...
        """
        return respond(blocked, "Appeal already submitted", 200)
...
async def submit(
    ...
    for token_h in stale_sessions:
        _used_sessions.pop(token_h, None)
